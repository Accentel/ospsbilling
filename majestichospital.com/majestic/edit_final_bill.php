...
<?php */?>	<?php
			//include("main_menu.php");
			$id=$_REQUEST['id'];
			$q=mysqli_query($link,"select * from final_bill where BILL_NO='$id'") or die(mysqli_error($link));
        ...
        <form name="cart" method="post" action="update_finalbill.php">
...
<input type="hidden" name="authby" value="<?php echo $aname ?>"/>
   ...
   <table width="100%" border="0" cellspacing="3" cellpadding="0">
        ...
        <?php 
		$k=mysqli_query($link,"select * from final_other_desc where fid='$id'") or die(mysqli_error($link));
...
<th class="THH1"><input name="new" type="button" class="butnbg1" value="  +  " onclick="javascript:addRow()">&nbsp; </th>
 ...
 <?php 
		$k=mysqli_query($link,"select * from final_doctor_visit where fid='$id'") or die(mysqli_error($link));
		...
		?>
 <tr>
 <td><div align='center' ><select  name='docname[]' id='docname' class='textbox1 print-type'><option value=''>Select Doctor Name </option><?php $p=mysqli_query($link,"select * from doct_infor");while($row=mysqli_fetch_array($p)){
	 $dd=$row['id'];
