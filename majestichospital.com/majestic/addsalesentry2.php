...

{
	...
	$rsp=mysqli_query($link,"select distinct product_name from phr_purinv_dtl where balance_qty>0")or die(mysqli_error($link));
...
var sum=0;
 var totaldiscount=0;
	...
	document.getElementById("nettotal").value=vall;
	//document.getElementById("nettotal").value=vall;
	//alert(vall);
...
function discal(str,id){
	//document.getElementById("total").value=vall;
	//alert(vall);
	var krk=document.getElementById("total").value;
	var krk1=document.getElementById("nettotal").value;
	//alert(krk1);
var value=document.getElementById("value"+id).value;
...
var vat=document.getElementById("va"+id).value;
if(vat=='' || vat==null){vat=0;}
document.getElementById("amt"+id).value=Math.abs((eval(disamt))+(eval(disamt))*(eval(vat))/100);
...
function amount_cal()
{
     //alert("hai");
     var cal=0;
	 var cal1=document.getElementById("nettotal").value;
	...
	{ 
		...
		document.getElementById("bal"+str).value=strar[1];
			 ...
			 // document.getElementById("pcode"+str).value=strar[5];
			   document.getElementById("va"+str).value=strar[5];
...
return false;
		...
		var actb4=document.getElementById("rows").value;
  ...
  //  oCell.innerHTML = "<input  type='text' name='pname"+rowCount+"' id='pname' class='textbox1' size='22'  onblur=showBatch(this.value,"+actb5+")>";
	oCell = document.createElement("TD");
	oCell.innerHTML = "<select name='pname"+rowCount+"' onblur='showBatch(this.value,"+actb5+")'  style=width:150px id='pname"+actb5+"'><option>Select Product</option><?php $a=mysqli_query("select product_name from phr_purinv_dtl");while($r=mysqli_fetch_array($a)){$k=$r['product_name'];?><option><?php echo $k?></option><?php }?></select></div>"; 
	...
	oCell = document.createElement("TD");
	oCell.innerHTML = "<input  type='text' name='vat"+rowCount+"' id='vat"+actb5+"' readonly class='textbox1'  size='7'>"; 
    row.appendChild(oCell);
...
function saletype(str){
	...
	<div id="conteneur" style="height:auto;">
		...
		<?php
			...
			//$optype=$_GET['optype'];
			 $mrnum=$_GET['mrnum'];
			$sq=mysqli_query("select * from 	`patientregister` where registerno='$mrnum'");
            ...
            <tr>
              <td><table width="100%" border="0" cellspacing="5" cellpadding="2">
                  ...
                  <tr>
                    ...
                    <td align="left"><!--<input type="radio" name="custm_type" id="custm_type1" value="c" onclick="javascript:custom(this.value)"/>
                      Customer-->
                        ...
                        <input type="radio" name="custm_type" id="custm_type3" checked="checked" value="p1" onclick="javascript:custom(this.value)"/>
                       OP Patients
                      </td>
                   </tr>
                  
				 <?php /*?><tr  style="display:none" id="trcust">
				 <td class="label1"><div align="right">Customer Name : </div></td>
                    <td align="left"><input name="custname1" type="text" class="text" id="custname1"/></td>
				   ...
				   <tr  style="display:none" id="trcust1">
					  ...
					  <td ><select name="csex" style="width:230px;height:22px;" id="csex"><option selected="selected">-- Sex --</option><option value="Male">Male</option><option value="Female">Female</option></select>   </td>
					   <td class="label1"><div align="right">Consultant Name : </div></td>
					  <td >
					  <select name="cconsultant_name" type="text" style="width:230px;height:22px;" id="cconsultant_name" >
						<option value="">Select Doctor Name</option>
                       <?php  
					   //include_once("config1.php");
					   $sqls=mysqli_query("SELECT * FROM doct_infor order by id asc");
				...
				?>
                  ...
                  <tr style="display:none" id="trpat">
					 ...
					 <select name="custname3" style="width:230px;height:22px;" id="custname3"  onchange="patientdetails(this.value)">
                       <option selected="selected">-Select In Patient-</option>
                       <?php
					   //include_once("config1.php"); 
							$sql = mysqli_query("select pat_no,patientname,a.registerno,b.PAT_REGNO from patientregister as a,ip_pat_admit as b where a.registerno=b.pat_regno and upper(dis_status) not like upper('discharged%')");
              ...
              <?php */?>    
                   <tr >
                     ...
                     <input type="text" name="" value="(<?php echo $mrnum?>)<?php echo $name?>" class="text" />
                      <input type="hidden" name="custname4" value="<?php echo $mrnum?>" class="text" />
                   ...
                   <?php /*?>  <select name="custname4" style="width:230px;height:22px;" id="custname4"  onchange="patientdetails1(this.value)">
                       ...
                       <?php 
					   //include_once("config1.php"); 
					   $xx="select PAT_REGNO,patientname,a.registerno from patientregister as a,`op_pat_dlt` as b where a.registerno=b.pat_regno";
							$sql = mysqli_query($xx);
					  ...
					  <td ><input name="custname2"  type="hidden" class="text" size="10" id="krk1" readonly/> 
                        <input name="agek" type="text" value="<?php echo $age?>" class="text" size="10" id="krk" readonly/>    </td>
                  </tr>
				 ...
				 <tr >
				  <td class="label1"><div align="right">Sex:</div></td>
					  <td ><input type="text" name="sexk"  value="<?php echo $sex?>" class="text" id="sex" readonly>   </td>
					  <td class="label1"><div align="right">Consultant Name:</div></td>
					  <td ><input name="consultant_namek" type="text"  value="<?php echo $doctorname?>" class="text" size="20" id="consultant_name"  readonly/>    </td></tr>
                  <tr style="display:none">
                    ...
                    <td style="display:none" id="conce3"><div align="left">
                        <select name="concessiontype" style="width:230px;height:22px;" id="concessiontype" onchange="javascript:aaa(this.value)">
                          <option value="General" selected="selected">General</option>
                          <?php
						  
							$sql1 = mysqli_query(link,"select CONCE_CODE, CONCE_NAME from CONCESSION_TYPE where CONCE_NAME != 'TPA Insurances'")or die(mysqli_error($link));
                  ...
                  <tr>
                    ...
                    <td width="27%" align="left"><select name="btype" style="width:230px;height:22px;" id="btype" >
					...
					<td align="left" ><select name="stype"  id="stype"  style="width:230px;height:22px;" onchange="javascript:saletype(this.value)">
                  ...
                  <tr>
                    ...
                    <td><div align="left">
                        <input name="saledate" type="text" class="tcal" style="width:230px;height:20px;" id="saledate" value="<?php echo date('d-m-Y') ?>"/>
                    </div></td>
                      ...
                      <table width="93%" class="ruler" id="dataTable" summary="Table of my records" align="center" >
                          ...
                          <tbody>
                              <tr>
                                  ...
                                  <td><input type='text' name='pname1' id='pname1' class='textbox1' onblur="showBatch(this.value,1)" size='22'/></td>
                                  <td><div id='batchdiv1'><select name='bachno1' class='select' style="width:75px" id='bachno1'><option>Batch</option></select></div></td>
                                  ...
                                  <td><input  type='text' name='amt1' id='amt1' class='textbox1' size='5'/><input  type='hidden' name='pcode1' id='pcode1' class='textbox1' size='5'/></td>
                                  <script type="text/javascript">
									//alert(customarray1);
                  ...
                  <tr>
                    <td align="center">
                         <table width="93%" class="ruler" id="dataTable" summary="Table of my records" align="center" >
						  ...
						  $x=0;
	$i=1;
						  $d="SELECT * FROM `dischargemedical` where mid='$idd' "; 
						   $fff=mysqli_query($link,$d)or die(mysqli_error($link));
						   while($r1=mysqli_fetch_array($fff)){
							   $i1=$i;
		  $x=$x+1;
						$mname=$r1['mname'];?>	   
                              ...
                              <tr>
                                  <td><input type="checkbox" id="checkbox1" name="checkbox1" value="1"/></td>
                                  <td>
								   <select name="pname<?php echo $x ?>" id="pname<?php echo $x ?>" onblur="showBatch(this.value,<?php echo $x?>)" style="width:150px;">
								  <option value="">Select Product</option>
								
								  <option value="<?php echo $mname?>"><?php echo $mname?></option></select>
								  
								  <!--<input type='text' name='pname1' id='pname' class='textbox1' onblur="showBatch(this.value,1)" size='22'/>--></td>
                                  <td><div id='batchdiv<?php echo $x ?>'><select name='bachno<?php echo $x ?>' class='select' style="width:75px" id='bachno<?php echo $x ?>'><option>Batch</option></select></div></td>
                                  <td><input  type='text' name='bal<?php echo $x ?>' id='bal<?php echo $x ?>' class='textbox1' size='7'/></td>
                                  <td><input  type='text' name='mfg<?php echo $x ?>' id='mfg<?php echo $x ?>' class='textbox1' size='7'/></td>
                                  <td><input  type='text' name='exp<?php echo $x ?>' id='exp<?php echo $x ?>' class='textbox1' size='7'/></td>
                                  <td><input  type='text' name='sqty<?php echo $x ?>' id='sqty<?php echo $x ?>' class='textbox1' size='5' onkeyup='val(this.value,<?php echo $x ?>)'/></td>
                                  <td><input  type='text' name='ucost<?php echo $x ?>' id='ucost<?php echo $x ?>' class='textbox1' size='5'/></td>
								   
                                  <td><input  type='text' name='value<?php echo $x ?>' id='value<?php echo $x ?>' value='0' class='textbox1' readonly size="10"/></td>
                                  <td><input  type='text' name='dis<?php echo $x ?>' id='dis<?php echo $x ?>' class='textbox1' value="0"  onkeyup='discal(this.value,<?php echo $x ?>)' size='5'/></td>
								   <td><input  type='hidden' name='va<?php echo $x ?>' id='va<?php echo $x ?>' class='textbox1' size='7'/>
								   <input  type='text' name='vat<?php echo $x ?>' id='vat<?php echo $x ?>' class='textbox1' size='7'/>
								   </td>
                                  <td><input  type='text' name='amt<?php echo $x ?>' id='amt<?php echo $x ?>' class='textbox1' size='5'/><input  type='hidden' name='pcode1' id='pcode1' class='textbox1' size='5'/></td>
                                  <script type="text/javascript">
                                  	var obj = actb2(document.getElementById('pname<?php echo $x ?>'),customarray1);
                                  </script>
                                  <input type="hidden" name="x" id="x" value="<?php echo $i ?>"/>
                                 
                              </tr><?php $i++;  }?>
                               <input type="hidden" name="rows" id="rows" value="<?php echo $i-1; ?>" onclick="javasript:noitems()"/>
...
NotPaid</td>
                          ...
                          <tr>
                            ...
                            <td width="19%" align="left"><input name="amount" type="text" class="text" id="amount" onkeyup='amount_cal();' value="0"/></td>
							</tr>
							<tr id="remaintr" style="display:''">
							<td width="85%" class="label1"><div align="right"> Remaining Change:</div></td>
                            <td width="19%" align="left"><input name="amount1" type="text" class="text" id="amount1" onblur='amount_cal();' value="0"/></td>
							</tr>
	  ...
	  <input name="actb" id="actb"  type="hidden" value="1"/>
	   <input name="rowval" id="rowval"  type="hidden" value="0"/>
                    ...
                    <td ><div align="center">
					 <input type="submit" value="Save" class="butt" onclick="save(0)" />&nbsp;<input type="button" value="Close" onclick="window.location.href='opdigitallab_reg1.php'" class="butt"/></div></td>
                  </tr>
              </table></td>
            </tr>
          </table>
