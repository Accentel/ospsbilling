...
document.getElementById("rbal").value=rbal;
	...
	<div id="conteneur">
		...
		<?php
		$q=$_REQUEST['lr_id'];
		$sql="select * FROM phr_salent_mast WHERE LR_NO = '$q' ";
//$sql="SELECT * FROM bill WHERE BillNO = '$q'";
$result = mysqli_query($link,$sql)or die(mysqli_error($link));
if($result){
	$row = mysqli_fetch_array($result);
	$b=$row['sr_bill_num'];
	 $date=date('d-m-Y',strtotime($row['SAL_DATE']));
	 $pname=$row['CUST_NAME'];
	$age=$row['age'];
	$gender=$row['sex'];
	$doctr=$row['Consultant_Name'];
	$total=$row['total'];
	$disc=$row['discount'];
	$met=$total-$disc;
	$paid=$met;
	...
	$pname2 = substr("$pname", 0, 2); 
	if($pname2=="MH"){
	    $p=mysqli_query($link,"select patientname  from patientregister where registerno='$mrno'") or die(mysqli_error($link));
	    $p1=mysqli_fetch_assoc($p);
	    $pname=$p1['patientname'];
...
?>
          <div align="center" style="font-size:20px;color:red;font-weight:bold;margin-bottom:20px;">Sales Due Bill</div>
		  <form name="frm" method="post" action="insertsale_due_bill.php">
			<table width="100%" border="0" cellpadding="4" cellspacing="0">
					...
					<tr >
                        ...
                        <td align="left" >
							...
							<input type="hidden" name="user1" value="<?php echo $_SESSION['suguna']; ?>"/>
                        </td>
						
                    </tr>
           
        </table>
		 <div align="center"><input type="submit" name="submit" value="Save" Class="butt"/> <input type="button" value="Close" Class="butt" onclick="window.location.href='salesentry_list.php'"/></div>
