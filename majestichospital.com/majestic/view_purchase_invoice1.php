...
$lr_id=$_REQUEST['id'];
$sql = mysql_query("select a.suppl_code,b.suppl_name,b.addr1,pur_type,suppl_inv_no,inv_date,rec_by,total,rec_date,grn,b.city,lr_no from phr_purinv_mast1 as a,phr_supplier_mast as b where lr_no='$lr_id' and a.suppl_code=b.suppl_code");
if($sql){
...
$sql1 = mysql_query("select product_code,product_name,packing_type,batch_no,mrp,exp_date,s_qty,s_free,s_rate,discount,value,vat,vat_amt,mfg_date,noitems,cost,manu_by,inv_id,sgst,cgst from phr_purinv_dtl1 where lr_no='$lr_id'");
...
$sql = mysql_query("select max(lr_no) from phr_purinv_mast1");
if($sql)
{
	$row = mysql_fetch_array($sql);
	$pnid = $row[0];
...
<table width="100%" >
              ...
              <input type="button" value="Close" class="butt" onclick = "window.location.href='purchase_invoice_list1.php'" > 
            </div>
			</form>
</div>
</div>
