  ...
  if (x) {
	...
	<div id="conteneur">
		...
		<div id="centre" style="height:auto;">
          <h1 style="color:red;" align="center">IN PATIENT ENQUIRY</h1>
...
<tr height="25px"><TH>S.NO.</TH><TH>PATIENT UMR No.</TH><TH>PATIENT NO.</TH><TH>PATIENT NAME</TH><TH>AGE/SEX</TH><TH>ADMISSION DATE</TH><TH>ROOM NO.</TH><TH>BED NO.</TH><TH>Discharge Status.</TH><TH>Direct Discharge</TH></tr>
<?php 
			  include("config.php");
			  ...
			  $r=$_REQUEST['reg'];
				...
				if($n != "" && $r != "")
				{
					...
					$sq=mysqli_query($link,"select a.PAT_NO,m.patientname,m.age,m.gender,a.ADMIT_DATE,a.DIS_STATUS,b.ROOM_NO,a.BED_NO,m.registerno FROM ip_pat_admit as a,patientregister as m,bed_details as b WHERE a.PAT_REGNO=m.registerno AND a.BED_NO=b.BED_NO and upper(a.dis_status) not like upper('discharged') and m.registerno='$r' and m.patientname='$n' order by a.ADMIT_DATE desc")or die(mysqli_error($link));
				}
			    else if($n != "")
				{
				$f="select a.PAT_NO,m.patientname,m.age,m.gender,a.ADMIT_DATE,a.DIS_STATUS,b.ROOM_NO,a.BED_NO,m.registerno FROM ip_pat_admit as a,patientregister as m,bed_details as b WHERE a.PAT_REGNO=m.registerno AND a.BED_NO=b.BED_NO and upper(a.dis_status) not like upper('discharged') and m.patientname='$n' order by a.ADMIT_DATE desc";
					$sq=mysqli_query($link,$f) or die(mysqli_error($link));;
				
				}
			    else if($r != "")
				{
					$sq=mysqli_query($link,"select a.PAT_NO,m.patientname,m.age,a.DIS_STATUS,m.gender,a.ADMIT_DATE,b.ROOM_NO,a.BED_NO,m.registerno FROM ip_pat_admit as a,patientregister as m,bed_details as b WHERE a.PAT_REGNO=m.registerno AND a.BED_NO=b.BED_NO and upper(a.dis_status) not like upper('discharged') and m.registerno='$r' order by a.ADMIT_DATE desc")or die(mysqli_error($link));
             ...
             <td><?php echo $d;?></td><td><?php echo $DIS_STATUS;?></td><td><a href="directdischarge.php?ipno=<?php echo $a ?>&bed=<?php echo $d ?>" onclick="return ConfirmDialog();" ><input type="button" class="button1" value="Discharge"></a></td></tr>
             <?php $i++;}
			 }
			 }
			 else{
			     $y="select a.PAT_NO,m.patientname,m.age,m.gender,a.ADMIT_DATE,a.DIS_STATUS,b.ROOM_NO,a.BED_NO,m.registerno FROM ip_pat_admit as a,patientregister as m,bed_details as b WHERE a.PAT_REGNO=m.registerno AND a.BED_NO=b.BED_NO and (a.DIS_STATUS) not like upper('discharged') order by a.ADMIT_DATE desc";
			...
			if($sq){	
			 ...
			 ?>
             <tr height="25px"><td style="text-align:center;"><?php echo $i;?></td><td><?php echo $e;?></td><td><?php echo $a;?></td><td><?php echo $h;?></td><td><?php echo $age."/".$gender;?></td><td><?php echo $b;?></td><td><?php echo $rno;?></td>
             <td><?php echo $d;?></td> <td><?php echo $DIS_STATUS;?></td><td><a href="directdischarge.php?ipno=<?php echo $a ?>&bed=<?php echo $d ?>" onclick="return ConfirmDialog();" ><input type="button" class="button1" value="Discharge Now"></a></td></tr>
             <?php }$i++;
			 }
			 }
			 }
