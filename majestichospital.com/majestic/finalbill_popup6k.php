...
function stateChanged(){ 
	...
	//alert('<%=current_date%>')
			opener.document.getElementById("bloodmrno").value=strar[1];
			opener.document.getElementById("patname").value=strar[2];
			opener.document.getElementById("patage").value=strar[3];
			opener.document.getElementById("patsex").value=strar[4];
			opener.document.getElementById("patno").value=strar[5];
			opener.document.getElementById("patmrno").value=strar[6];
			opener.document.getElementById("phynm").value=strar[7];
...
<tr height="25px"><TH>PATIENT REG NO.</TH><TH>PATIENT UMR No.</TH><TH>PATIENT NAME</TH></tr>
<?php 
			  include("config.php");
			   ...
			   if(isset($_REQUEST['submit'])){
			    ...
			    if($n != "")
				{
					$sq=mysql_query("select distinct a.PAT_NO,b.patientname,b.registerno from ip_pat_admit as a,patientregister as b 
					WHERE a.pat_regno=b.registerno and patientname='$n'");
				}
			   if($sq)
			   {
