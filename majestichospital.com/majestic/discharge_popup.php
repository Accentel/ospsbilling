...
function stateChanged(){ 
	...
	//alert('<%=current_date%>')
			...
			opener.document.getElementById("patname").value=strar[3];
			opener.document.getElementById("age").value=strar[4];
			opener.document.getElementById("sex").value=strar[5];
			opener.document.getElementById("admit").value=strar[6];
			opener.document.getElementById("discharge").value=strar[7];
			opener.document.getElementById("relation").value=strar[8];
			opener.document.getElementById("doctor").value=strar[9];
			opener.document.getElementById("addr").value=strar[11];
...
<?php 
			  include("config.php");
			   ...
			   if(isset($_REQUEST['submit'])){
			    ...
			    if($n != "")
				{
					$sq=mysql_query("select distinct a.PAT_NO,b.patientname,b.registerno from ip_pat_admit as a,patientregister as b WHERE a.pat_regno=b.registerno and patientname='$n'");
				}
			   if($sq)
			   {
			...
			if($sq){	
             ...
             <input type="radio" name="empid" value="<?php echo $a ?>" onclick="javascript:showDoc(this.value);"/></td><td><?php echo $a ?></td><td><?php echo $h;?></td></tr>
             <?php }
