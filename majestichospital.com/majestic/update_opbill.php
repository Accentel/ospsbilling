...
echo $q="update  opbill set  BillNO='$bno', BillDate='$bdate', PatientName='$pname', Age='$age', Sex='$gender', DoctorName='$dname',Total='$total',Discount='$disc',NetAmount='$namt',PaidAmount='$paid',BalanceAmount='$bal',conce_type='$ctype',ptype='$ptype',remarks='$remarks',user='$user',phoneno='$mno',mrno='$mrno',time='$time',bstatus='$status',bsdate='$sdate'  where id='$id2'";
...
$id=$_POST['id'][$i];
if($id != "" and $tname!=''){
$sql1 = mysqli_query($link,"update  opbill1 set BillNO='$bno', BillDate='$bdate',purpose='$tname',Amount='$cost', mrno='$mrno' where id='$id'")or die(mysqli_error($link));
}else{
	if($tname!=''){
	$sql1 = mysqli_query($link,"insert into opbill1(BillNO, BillDate,purpose,Amount, mrno) values('$bno','$bdate','$tname','$cost','$mrno')")or die(mysqli_error($link));
	}
	}
