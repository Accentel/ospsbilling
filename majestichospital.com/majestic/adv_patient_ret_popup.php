  ...
  function showDoc(str){
		 ...
		 xmlHttp=GetXmlHttpObject();
		  if (xmlHttp==null){
			  ...
			  var url="adv_patient_popup2.php"
			  url=url+"?emp_id="+str;
		...
		//alert(strar);
		  ...
		  if(strar.length>0){
			 ...
			 //window.location.reload(); 
			  ...
			  opener.document.getElementById("rupees").value=strar[6];
			  opener.document.getElementById("adv_paid").value=strar[7];
			   opener.document.getElementById("adv_paid1").value=strar[8];
...
<?php
	...
	if(isset($_REQUEST['searchname']))
	{
		...
		$sql = mysqli_query($link,"select distinct a.bill_num,a.pat_no,patientname,age,gender,admit_date from ip_pat_admit as a,patientregister as b,adv_collection as c where a.pat_regno=b.registerno and a.pat_no=c.PAT_NO  and b.patientname='$n' order by a.pat_no desc ")or die(mysqli_error($link));
		$rows = mysqli_num_rows($sql);
	}
	else
	{
		$sql = mysqli_query($link,"select distinct a.bill_num,a.pat_no,patientname,age,gender,a.admit_date,ADV_AMNT from ip_pat_admit as a,patientregister as b,adv_collection as c where a.pat_regno=b.registerno and a.pat_no=c.PAT_NO   order by a.pat_no desc ")or die(mysqli_error($link));
		$rows = mysqli_num_rows($sql);
	}
?>
...
<table width="400px" class="sortable"  id="TABLE1" align="center">
  ...
  <tbody class="box_midlebg">
	...
	?>
    <tr class="tr1">
      <td class="TD1"><input type="radio" name="empid" value="<?php echo $res[1] ?>" onclick="javascript:showDoc(this.value);"/> <?php echo $res[1] ?>       </td>
      <td class="TD1"><?php echo $res[2] ?></td>
    <td class="TD1"><?php echo date('d-m-Y',strtotime($res[5])) ?></td>
	<td class="TD1"><?php echo $res['bill_num'];?></td>
    </tr>
	<?php
		}
	}
