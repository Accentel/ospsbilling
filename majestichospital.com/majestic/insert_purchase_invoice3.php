...
if($_REQUEST['pname'][$i]!=""){
if(($inv_id=$invid)){
 $pp="update  phr_purinv_dtl set   product_name='$pname',batch_no='$batch',mrp='$mrp',exp_date='$expdt',s_qty='$sqty',s_free='$sfree',s_rate='$srate',value='$value',vat='$vat',vat_amt='$vatamt',currentdate=now(),auth_by='$admin',mfg_date='$mfgdt',noitems='$noi',balance_qty='$bal_qty',manu_by='$maniby',HSN='$HSN' where lr_no='$lrno' and inv_id='$invid' " or die(mysqli_error($link));
$sql1=mysqli_query($link,$pp)or die(mysqli_error($link));
 ...
 values($lrno,'$pname','$batch','$mrp','$expdt','$sqty','$sfree','$srate','$value','$vat','$vatamt',now(),'$admin','$mfgdt','$noi','$bal_qty','$maniby','$sgst','$cgst','$HSN')" or die(mysqli_error($link));
