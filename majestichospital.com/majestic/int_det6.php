...
window.open('view_in_patient_admit10.php?adv_no='+b+'','mywindow','width=700,height=500,toolbar=no,menubar=no');
	}
...
<?php /*?>	<div id="conteneur">
		...
		<?php
			...
			?><?php */?>
		<div id="centre">
          <h1 style="color:red;" align="center">SUGAR CHART</h1>
...
<?php 
			 ...
			 //$n=$_REQUEST['name'];
			  $r=$_REQUEST['reg'];
					...
					//$s="SELECT a.id,a.mrnum,b.patientname,c.ADMIT_DATE,b.age,b.gender FROM casesheet_sugarchart a,patientregister b,`ip_pat_admit` c 
					//where a.mrnum=b.registerno and c.PAT_REGNO=b.registerno and a.pat_regno=c.PAT_REGNO and a.mrnum='$r' order by a.id desc";
				
				  $s="SELECT a.id,a.pat_regno,b.patientname,c.ADMIT_DATE,b.age,b.gender FROM casesheet_sugarchart a,patientregister b,`ip_pat_admit` c 
					where a.pat_regno=b.registerno and c.PAT_REGNO=b.registerno and a.pat_regno=c.PAT_REGNO  and a.pat_regno='$r' order by a.id desc";
             ...
             <?php }$i++;}
				 ...
				 //from ip_pat_admit as A,patientregister as M WHERE  A.PAT_REGNO=M.registerno and dis_status not like 'Discharged' order by reg_id desc";
				  $s="SELECT a.id,a.pat_regno,b.patientname,c.ADMIT_DATE,b.age,b.gender FROM casesheet_sugarchart a,patientregister b,`ip_pat_admit` c 
					where a.pat_regno=b.registerno and c.PAT_REGNO=b.registerno and a.pat_regno=c.PAT_REGNO order by a.id desc";
				$sq=mysqli_query($link,$s)or die(mysqli_error($link));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<td width="25" align="right"><?php if (!($pagecount == 0)) { ?><a href="int_det6.php?next=0" ><?php } ?><img src="images/first.gif" title="First" alt="First" width="16" height="14" border="0"/></a></td>
		<td width="10">&nbsp;</td>
	   
		<td width="25" align="right"><?php if (!($pagecount == 0)) { ?>
			  <a  href="int_det6.php?next=<?php echo ($pagecount - 1) ?>"><?php } ?><img src="images/previous.gif" title="Previous" alt="Previous" width="16" height="14" border="0"/></a></td>
		  <td width="10">&nbsp;</td>
	  
		<td width="25" align="right"><?php if ($rowscounts > 0) { ?>
			  <a href="int_det6.php?next=<?php echo ($pagecount + 1) ?>"><?php } ?><img src="images/next.gif" title="Next" alt="Next" width="16" height="14" border="0"/></a></td>
		<td width="10">&nbsp;</td>
		<td width="25" align="right"><?php if ($rowscounts > 0) { ?><a href="int_det6.php?next=<?php echo (($records - 1) / $nd) ?> >  <?php } ?><img src="images/last.gif" title="Last" alt="Last" width="16" height="14" border="0"/></a></td>
	  </tr>
	</table>
	<table>
	<?php if ($rowscounts == 0) { ?>
