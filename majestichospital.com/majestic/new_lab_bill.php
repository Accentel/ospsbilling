						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								...
								if($prdname != "" & $pname != ""){
								 ...
								 $s="select distinct mrno, BillNO,PatientName,ptype,NetAmount,PaidAmount,BalanceAmount from bill1 where BillNO='$prdname' order by bid desc";
									$sql = mysqli_query($link,$s)or die(mysqli_error($link));
								
								}elseif($pname != ""){
								 $s1="select distinct mrno,BillNO,PatientName,ptype,NetAmount,PaidAmount,BalanceAmount from bill1 where PatientName='$pname' ";
							    ...
							    $bdate=date('Y-m-d');
							    if($name!='admin'){
							        $sql = mysqli_query($link,"select distinct  a.mrno,a.BillNO,a.PatientName,a.NetAmount ,a.PaidAmount,a.BalanceAmount,a.ptype  from bill1 a where user='$name' and a.BillDate='$bdate'   order by a.bid desc limit 150")or die(mysqli_error($link));
							    }else{
								$sql = mysqli_query($link,"select distinct  a.mrno,a.BillNO,a.PatientName,a.NetAmount ,a.PaidAmount,a.BalanceAmount,a.ptype  from bill1 a  where a.BillDate='$bdate'  order by a.bid desc")or die(mysqli_error($link));
							    }
							}
							if($sql){
