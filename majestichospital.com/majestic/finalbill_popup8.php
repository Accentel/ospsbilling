...
function stateChanged(){ 
	...
	//alert('<%=current_date%>')
			opener.document.getElementById("reg").value=strar[1];
			opener.document.getElementById("pname").value=strar[2];
			opener.document.getElementById("motaddress").value=strar[3];
...
<?php 
			  include("config.php");
			   ...
			   if(isset($_REQUEST['submit'])){
			    ...
			    if($n != "")
				{
					$sq=mysqli_query($link,"select distinct a.PAT_NO,b.patientname,b.registerno from ip_pat_admit as a,patientregister as b 
					WHERE a.pat_regno=b.registerno and patientname='$n'")or die(mysqli_error($link));
				}
			   if($sq)
			   {
