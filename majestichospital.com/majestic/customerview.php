...
$().ready(function() {
	...
	<div id="conteneur">
		<?php include("logo.php");?><?php
			...
			?>
		<div id="centre">
          <h1 style="color:red;" align="center">CUSTOMERS LIST</h1>
          
                       <form name="frm" method="post" >
           
                
<table align="right" cellspacing="2">
<tr><td>Search By Customer Name : <!--<input type="text" name="name" id="name" required/>-->

<input id=\"prd\" list=\"city1\" name="name"  class="text"required >
<datalist id=\"city1\" >

<?php  include("config.php");
$sql="select distinct cname from customer ";  // Query to collect records
$r=mysqli_query($link,$sql) or die(mysqli_error($link));
while ($row=mysqli_fetch_array($r)) {
echo  "<option value=\"$row[cname]\"/>"; // Format for adding options 
}
...
<tr><td width="68" class="label1"><a href="add_customer.php" title="Add New Record"><img src="images/add1.gif"></a></td></tr>
</table>
<table width="100%" cellpadding="0" cellspacing="0" border="0" id="expense_table" style="font-size:14px;">
<tr height="25px"><TH>S.NO</TH><TH>CUSTOMER NAME</TH><TH>ADDRESS</TH><TH>MOBILE</TH><TH>VIEW</TH></tr>
<?php 
			  include("config.php");
			   if(isset($_REQUEST['submit'])){
			  $d=$_REQUEST['name'];
			  
			  $sq = mysqli_query($link,"select * from  customer where cname='$d'")or die(mysqli_error($link));
			  ...
			  $records++;
				...
				if ($rowstart <= $rowend && $remainig_records == $rowstart) {
					...
					$rowscounts = 0;
			 ?>
             <tr height="25px"><td style="text-align:center;"><?php echo $i;?></td><td><?php echo $depname;?></td><td><?php echo $mdepname;?></td><td><?php echo $locname;?></td><td style="text-align:center;"><a href="edit_customer.php?id=<?php echo $did?>"><img src="images/edit.gif" /></a></td></tr>
             <?php $i++;}}}
			 }else{
				$sq = mysqli_query($link,"select * from customer")or die(mysqli_error($link));
			  $tot=mysqli_num_rows($sq);
			  $i = 1;
			  if($sq){
			  while($res=mysqli_fetch_array($sq)){
