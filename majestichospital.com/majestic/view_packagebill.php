...
$().ready(function() {
	...
	<div id="conteneur">
		...
		<div id="centre" style="height:420px;">
         ...
         <div align="center" style="font-size:20px;color:red;font-weight:bold;margin-bottom:5px;">View Extra Package Services Cash Bill</div>
                      ...
                      <tbody>
						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								...
								if($prdname != "" & $pname != ""){
									$sql = mysqli_query($link,"select distinct mrno,BillNO,PatientName,BillDate,NetAmount,PaidAmount,BalanceAmount from packagebill where mrno='$prdname' and PatientName='$pname' order by id desc")or die(mysqli_error($link));
								}elseif($prdname != ""){
								 $s="select distinct mrno,BillNO,PatientName,BillDate,NetAmount,ptype,PaidAmount,BalanceAmount from packagebill where mrno='$prdname' order by id desc";
									$sql = mysqli_query($link,$s)or die(mysqli_error($link));
								
								}
								elseif($pname != ""){
								echo $s1="select distinct mrno,BillNO,PatientName,BillDate,NetAmount,ptype,PaidAmount,BalanceAmount from packagebill where PatientName='$pname' ";
								...
								$sql = mysqli_query($link,"select  * from packagebill   order by id desc")or die(mysqli_error($link));
							}
							if($sql){
						   ...
						   <tr>
							  ...
							  <td><?php echo $row['PaidAmount'] ?></td>
							   ...
							   <td><a href="edit_packagebill.php?bno=<?php echo $row['BillNO'] ?>"><img src="images/edit1.jpg" height="21"/></a></td>
							  <td><a href="#" onclick="window.open('packagebill_rec2.php?bno=<?php echo $row['BillNO'] ?>','mywindow','width=1020,height=700,toolbar=no,menubar=no,scrollbars=yes')" ><img src="images/green.png" height="22"/></a></td>
						  ...
						  <?php
							...
							if(isset($_REQUEST['submit'])){
								...
								if($prdname != "" & $pname != ""){
								 ...
								 $s="select distinct mrno, BillNO,PatientName,ptype,BillDate,NetAmount,PaidAmount,BalanceAmount from packagebill where mrno='$prdname' order by id desc";
									$sql = mysqli_query($link,$s)or die(mysqli_error($link));
								
								}
								elseif($pname != ""){
								 $s1="select distinct mrno,BillNO,PatientName,BillDate,ptype,NetAmount,PaidAmount,BalanceAmount from packagebill where PatientName='$pname' ";
								//echo $p="select distinct BillNO,PatientName,Total,PaidAmount,BalanceAmount from bill1 where PatientName='$pname' order by bid desc";
									$sql = mysqli_query($link,$s1)or die(mysqli_error($link));
							    ...
							    $tdate=date('Y-m-d');
								$sql = mysqli_query($link,"select  * from packagebill  where BillDate='$tdate'  order by id desc")or die(mysqli_error($link));
							}
							if($sql){
								$rows=mysqli_num_rows($sql);
								if($rows > 0){
